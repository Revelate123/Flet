...
def create_app(test_config=None):
  ...
  @app.errorhandler(google.auth.exceptions.GoogleAuthError)
  def handle_exception_google(e):
    """Return JSON instead of HTML for HTTP errors."""
    # start with the correct headers and status code from the error
    return render_template("restaurant.html",chat_reply="This is when I used to call the Google VertexAI API, but my free trial has since ended :(")

  @app.route('/test')
  def test():
     return flask.url_for('test', _external=True)
  ...
  def index():
    ...
    return flask.redirect(flask.url_for('restaurant',_external=True))
  ...
  def cv():
          ...
          return send_file('static/Thomas Duffett CV - Software Engineer.pdf', mimetype='.pdf')
